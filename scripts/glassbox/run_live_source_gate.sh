...
cargo test --manifest-path "$ROOT/Cargo.toml" -p glassbox-live-source
cargo clippy --manifest-path "$ROOT/Cargo.toml" -p glassbox-live-source --all-targets -- -D warnings
