...
EXTERNAL_SELF_TEST="$(mktemp "${TMPDIR:-/tmp}/glassbox-external-evidence.XXXXXX")"
TEMPLATE_OUT="$(mktemp "${TMPDIR:-/tmp}/glassbox-benchmark-template.XXXXXX")"
trap 'rm -f "$MYSTERY" "$SELF_TEST" "$PROMOTION_SELF_TEST" "$EXTERNAL_SELF_TEST" "$TEMPLATE_OUT"' EXIT
...
python3 "$ROOT/scripts/glassbox/external_evidence.py" --self-test >"$EXTERNAL_SELF_TEST"
...
python3 - "$ROOT" "$MYSTERY" "$SELF_TEST" "$PROMOTION_SELF_TEST" "$EXTERNAL_SELF_TEST" "$TEMPLATE_OUT" "$RECEIPT" <<'PY'
import json, pathlib, subprocess, sys
root,mystery_path,self_path,promotion_self_path,external_self_path,template_path,receipt_path=map(pathlib.Path,sys.argv[1:])
mystery=json.loads(mystery_path.read_text()); self_test=json.loads(self_path.read_text()); promotion_self_test=json.loads(promotion_self_path.read_text()); external_self_test=json.loads(external_self_path.read_text()); template=json.loads(template_path.read_text())
def git(*args):
 r=subprocess.run(["git",*args],cwd=root,text=True,capture_output=True); return r.stdout.strip() if r.returncode==0 else "unknown"
checks={"sixty_public_rehearsal_scenarios":mystery.get("scenario_count")==60,"public_corpus_not_mislabeled_held_out":mystery.get("checks",{}).get("public_corpus_explicitly_not_held_out") is True,"validator_self_tests_pass":self_test.get("ok") is True,"promotion_verifier_self_tests_pass":promotion_self_test.get("ok") is True,"external_evidence_self_tests_pass":external_self_test.get("ok") is True,"incomplete_external_template_fails_closed":template.get("ok") is False and bool(template.get("errors"))}
