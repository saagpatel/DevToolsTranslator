...
final class PassiveBrokerProcess: @unchecked Sendable {
  typealias MessageHandler = @Sendable (PassiveBrokerMessage) -> Void
  ...
  func start(
    ...
    input: PassiveCaptureInput,
    onMessage: @escaping MessageHandler,
    onExit: @escaping ExitHandler
  ) throws {
    lock.lock()
    defer { lock.unlock() }
    guard processID == nil else { throw PassiveProcessError.alreadyRunning }
    let evidenceDescriptor = open(evidenceURL.path, O_WRONLY | O_CREAT | O_EXCL, S_IRUSR | S_IWUSR)
    guard evidenceDescriptor >= 0 else { throw PassiveProcessError.outputOpenFailed }
    let requestInput = Pipe()
    let output = Pipe()
    let errors = Pipe()
    let consent = Pipe()
    // High-numbered child-only descriptors avoid colliding with descriptors
    // already owned by AppKit, Swift concurrency, or the pipes above.
    let childEvidence: Int32 = 198
    let childConsent: Int32 = 199
    var actions: posix_spawn_file_actions_t?
    guard posix_spawn_file_actions_init(&actions) == 0 else {
      close(evidenceDescriptor)
      throw PassiveProcessError.spawnSetupFailed
    }
    defer { posix_spawn_file_actions_destroy(&actions) }
    let mappings = [
      (requestInput.fileHandleForReading.fileDescriptor, STDIN_FILENO),
    ...
    let descriptors = Set([
      requestInput.fileHandleForReading.fileDescriptor,
      requestInput.fileHandleForWriting.fileDescriptor,
      ...
      consent.fileHandleForReading.fileDescriptor,
      consent.fileHandleForWriting.fileDescriptor,
      evidenceDescriptor,
    ...
    let operation = switch input {
    case .liveSnapshot: "--snapshot-evidence"
    case .fixture: "--parse-stdin-evidence"
    }
    let arguments = [
      helperURL.path,
      operation,
      "--evidence-fd=\(childEvidence)",
    ...
    close(evidenceDescriptor)
    requestInput.fileHandleForReading.closeFile()
    output.fileHandleForWriting.closeFile()
    errors.fileHandleForWriting.closeFile()
    consent.fileHandleForReading.closeFile()
    guard status == 0 else {
      requestInput.fileHandleForWriting.closeFile()
      ...
      throw PassiveProcessError.spawnFailed
    }
    processID = pid
    inputHandle = requestInput.fileHandleForWriting
    consentHandle = consent.fileHandleForWriting
    outputHandle = output.fileHandleForReading
    errorHandle = errors.fileHandleForReading
    _ = fcntl(requestInput.fileHandleForWriting.fileDescriptor, F_SETNOSIGPIPE, 1)
    ...
    do {
      ...
      request.append(0x0A)
      if case let .fixture(bytes) = input { request.append(bytes) }
      try requestInput.fileHandleForWriting.write(contentsOf: request)
      requestInput.fileHandleForWriting.closeFile()
    } catch {
      _ = kill(pid, SIGTERM)
      throw error
  ...
  private func consumeOutput(_ data: Data, onMessage: MessageHandler) {
    ...
    for line in lines where !line.isEmpty {
      if let message = try? JSONDecoder().decode(PassiveBrokerMessage.self, from: line) {
        onMessage(message)
      }
    }
  }
...
enum PassiveProcessError: LocalizedError {
  case alreadyRunning
  ...
  var errorDescription: String? {
    switch self {
    case .alreadyRunning: "A passive capture is already active."
    case .outputOpenFailed: "The selected evidence destination could not be created."
    case .spawnSetupFailed: "The isolated passive receiver could not be prepared."
    case .spawnFailed: "The isolated passive receiver could not be launched."
    }
  }
}
