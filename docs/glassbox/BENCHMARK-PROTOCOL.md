...
3. Use pilot variance for a power analysis; predeclare alpha, target power, and a minimum meaningful fractional reduction in time to first evidence-supported conclusion. Do not claim efficacy from the pilot.
4. Pre-register the formal sample size, exact primary outcome, meaningful-effect threshold, statistical test, exclusions, and stopping rule.
5. Run the held-out formal study with independent scoring and blinded adjudication of disagreements; record its completion time before independent verification begins.

Agents may build fixtures and scoring tools but do not substitute for human participants.

`scripts/glassbox/validate_benchmark_results.py` validates the required external artifact shape, frozen-candidate digest, record hashes, study completion time, and product thresholds. The preregistered and power-analysis primary outcome must both be exactly `time_to_first_evidence_supported_conclusion`; their meaningful-effect thresholds must match. Promotion requires a finite observed fractional time reduction at or above that threshold, a finite 95% confidence interval wholly above zero and containing the effect estimate, and a primary p-value below the preregistered alpha. A direction boolean and p-value alone are insufficient. Structural validation does not authenticate participant identity, preregistration, or study records; an independent reviewer must verify those primary sources only after the recorded formal-study completion time and sign a second payload bound to both the study artifact and the same candidate manifest. The signed payload must carry typed, relative-path, SHA-256 attachments for the participant registry, study records, scoring export, adjudication log, preregistration record, held-out corpus manifest, and metric recomputation output. Every attachment must be a distinct regular file confined beside the CMS envelope without absolute paths, traversal, or symlink components, and its digest must match the corresponding hash in the study artifact. Bare booleans or hash strings without the bound files cannot promote. `scripts/glassbox/run_benchmark_readiness_gate.sh` must report `benchmark_passed: false` and `gate6_promotable: false` until a genuine external artifact passes that review and validation.
