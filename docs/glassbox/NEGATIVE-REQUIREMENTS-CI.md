...
Gate 0's deterministic contract/source scanner is `scripts/glassbox/check_gate0.py`; CI runs it and its embedded failing self-test. CI also runs the pure self-tests for candidate identity, the exact notarization batch, immutable fresh-VM transfer, shared external-evidence authentication, human-benchmark structure and independent promotion, retirement evidence, and consolidated program readiness. Candidate, Apple-evidence, study-evidence, Gate 6 composition, or Gate 7 retirement drift therefore fails before merge. These host-neutral self-tests prove validator behavior only; they do not substitute for signed macOS artifacts, Apple responses, stapling, Gatekeeper, observed fresh-VM evidence, actual human participants, independent review, production releases, soak, or retirement authority. Gate 0 emits `glassbox-gate0-receipt/v2`. A `--precommit` receipt is explicitly provisional and may describe dirty or untracked draft files. Only an authoritative run from a clean commit, hashing required artifacts from `HEAD` and recording its tree ID, can promote the gate. Later gates extend the same fail-closed posture with executable IPC, import, key, redaction, egress, distribution, and uninstall suites at the paths named by their implementation plans.
