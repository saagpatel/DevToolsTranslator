...
|---|---|---|---|
| DevTools Translator | Tauri/React scaffold, evidence-resolution concepts, deterministic/canonicalization ideas, browser adapter fixtures | Existing normalized schema as canonical kernel; current loopback discovery; global request IDs; fail-open sanitizer; fixed ZIP importer | Becomes Glassbox shell after isolated kernel proof |
...
| Pulse Orbit | Bounded read-only resource sampler logic and fixtures | Alerts, history service, process killing, dashboard, unbounded all-process polling | Candidate retirement after sampler parity |
