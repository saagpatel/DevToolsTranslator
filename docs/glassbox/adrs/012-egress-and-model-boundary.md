...
The Native Messaging host communicates only over Chrome stdin/stdout and authenticated local XPC with the signed app. The loopback OTLP broker can bind only `127.0.0.1`/`::1`, requires a per-session attachment credential, enforces rate/byte/event limits, and has no outbound network entitlement. Passive local-network context is a separately signed, independently disabled, one-shot broker. It requires a per-invocation consent capability, executes only the fixed ordinary-access `/usr/sbin/arp -an` snapshot command, has no network or privileged entitlement, and cannot publish active-scan commands. Its output is untrusted logical context, never topology, ownership, attribution, or causal evidence.
